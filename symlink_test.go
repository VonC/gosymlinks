...
func TestDestination(t *testing.T) {
	...
	var sl *SL
	var err error
	sl, err = New("unknown/dst")
	if err == nil || strings.Contains(err.Error(), "The system cannot find the path specified") == false {
		t.Errorf("Err '%v', expected 'The system cannot find the path specified'", err)
	}
	if sl != nil {
		t.Errorf("SL '%v', expected <nil>", sl)
	}
	_, err = New(string([]byte{0}))
	if err == nil || strings.Contains(err.Error(), "invalid argument") == false {
		t.Errorf("Err '%v', expected 'invalid argument'", err)
	}
	New(".")
}
